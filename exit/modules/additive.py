...
class MOFOnlyRegressionHead(nn.Module):
    ...
    def forward(self, mof_emb):
        return self.net(mof_emb)

class XRDOnlyRegressionHead(nn.Module):
    def __init__(self, xrd_dim, hidden_dim=128, dropout=0.1):
        super().__init__()
        self.net = nn.Sequential(
    ...
    def forward(self, xrd_emb):
        return self.net(xrd_emb)
...
class MultiModalAdditiveRegressor(pl.LightningModule):
    def __init__(
        ...
        self.head_type = backbone_config.get("additive_head_type", "additive")
        self.mof_pooling_mode = backbone_config.get('mof_pooling_mode', 'mean')
        self.xrd_pooling_mode = backbone_config.get('xrd_pooling_mode', 'mean')
        
        if self.head_type == "mof_only":
            self.regression_head = MOFOnlyRegressionHead(
                mof_dim=hidden_dim,
                hidden_dim=head_hidden_dim,
                dropout=head_dropout,
            )
        elif self.head_type == "xrd_only":
            self.regression_head = XRDOnlyRegressionHead(
                xrd_dim=hidden_dim,
                hidden_dim=head_hidden_dim,
                dropout=head_dropout,
            )
        elif self.head_type == "additive":
            ...
            raise ValueError(f"Unknown additive_head_type: {self.head_type}")
        # -------------------------
        # target normalizer
        # -------------------------
    ...
    def pool_with_mode(self, feats, masks=None, mode="mean"):
        if mode == "mean":
            return masked_mean_pooling(feats, masks)
        elif mode == "first":
            return self.backbone.pooler(feats)
        else:
            raise ValueError(f"Unknown pooling mode: {mode}")        
    # -------------------------
    # feature extraction
    # -------------------------
    def extract_mof_only_features(self, batch):
        ...
        x = mofid_embeds
        x_masks = attention_mask
    
        for blk in backbone.vision_transformer.blocks:
            x, _ = blk(x, mask=x_masks)
    
        x = backbone.vision_transformer.norm(x)
        mof_pooled = self.pool_with_mode(x, attention_mask, mode = self.mof_pooling_mode)
        return mof_pooled
    
    def extract_xrd_only_features(self, batch):
        backbone = self.backbone
        device = self.device
        ...
        x = xrd_embeds
        x_masks = xrd_attention_masks
    
        for blk in backbone.vision_transformer.blocks:
            x, _ = blk(x, mask=x_masks)
    
        x = backbone.vision_transformer.norm(x)
        xrd_pooled = self.pool_with_mode(x, xrd_attention_masks, mode = self.xrd_pooling_mode)
        return xrd_pooled
    
    
    def extract_multimodal_features(self, batch):
        ...
        # pooling
        # -------------------------
        mof_pooled = self.pool_with_mode(mofid_feats, attention_mask, mode = self.mof_pooling_mode)
    
        xrd_pooled =self.pool_with_mode(
            xrd_feats,
            torch.ones(xrd_feats.shape[:2], device=device),
             mode = self.xrd_pooling_mode
        )
    ...
    def forward(self, batch):
        if self.head_type == "mof_only":
            mof_pooled = self.extract_mof_only_features(batch)
            pred = self.regression_head(mof_pooled).squeeze(-1)
    
        elif self.head_type == "xrd_only":
            xrd_pooled = self.extract_xrd_only_features(batch)
            pred = self.regression_head(xrd_pooled).squeeze(-1)
    
        elif self.head_type == "additive":
            mof_pooled, xrd_pooled = self.extract_multimodal_features(batch)
            pred = self.regression_head(mof_pooled, xrd_pooled).squeeze(-1)
