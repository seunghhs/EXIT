...
# Additive head
# =========================
class GatedAdditiveRegressionHead(nn.Module):
    def __init__(self, mof_dim, xrd_dim, hidden_dim=128, dropout=0.1):
        ...
        self.joint_head = nn.Sequential(
            nn.Linear(mof_dim + xrd_dim + inter_dim, hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
        ...
        self.gate = nn.Sequential(
            nn.Linear(mof_dim + xrd_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, mof_emb, xrd_emb):
        ...
        gate = self.gate(torch.cat([mof_emb, xrd_emb], dim=-1))
        correction = gate * (xrd_out + joint_out)

        return mof_out + correction
...
class MultiModalAdditiveRegressor(pl.LightningModule):
    def __init__(
        ...
        self.regression_head =  GatedAdditiveRegressionHead(
            mof_dim=hidden_dim,
            xrd_dim=hidden_dim,
