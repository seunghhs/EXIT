"""
Finetuning dataset for regression tasks (surface area or pore volume prediction).
Loads experimental MOF data (xrd, regression label, mofid) from a pickle file.
Used with DataCollatorForLanguageModeling(mlm=False) — no token masking applied.
"""
import os
import pickle
import numpy as np
    ...
    def __init__(self, data_dir: str):
        ...
        Args:
            data_dir: path to .pkl file. Each item must have keys:
                ...
                regression (float)                           — SA [m²/g] or PV [cm³/g] label
...
def custom_collate_fn(batch, data_collator):
    """
    Collate function for finetuning. With mlm=False, data_collator passes input_ids through
    unchanged and sets all labels to -100 (ignored in loss). Handles padding per-sample.
    """
    for item in batch:
