...
class BasicDataset(Dataset):
    def __init__(self, data_dir: str):
        ...
        self.xrd, self.classification, self.mofid, self.name, self.ref =\
        zip(*[(np.expand_dims(item['xrd'], axis=0), item['classification'],  item['mofid'], item['name'], item['ref']) for item in data_list])
        ...
        self.classification = torch.tensor(self.classification, dtype=torch.long)
